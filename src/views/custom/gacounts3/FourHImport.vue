				...
				<div v-if="!loadingActivity">
					<form>
						...
						<label class="checkbox-container">
							...
							<span>Were adults part of your target audience?</span>
						</label>
						<label class="activity">
							...
							<p>
								<em>Note: To select multiple actvities, hold down the CTL key as
									you select them.</em>
							</p>
							<select v-if="!loadingActivityList" v-model="activityIDs" :disabled="activities.length < 1"
								multiple :size="activities.length < 15 ? activities.length : 15">
								<option v-for="activity in activities" :key="activity.ACTIVITY_ID"
									:value="activity.ACTIVITY_ID" :class="
										activityAlreadyImported(activity.ACTIVITY_ID)
											? 'alreadyImported'
											: ''
									">
									{{ activity.NAME }} - ({{ simpleDate(activity.BEGIN_DATE) }})
						...
						<div class="button-container">
							<button type="button" :disabled="activityIDs.length < 1"
